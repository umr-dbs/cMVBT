...
# Reproduce Paper Results:
    ...
    3. Execute workloads via 'load <data-file> <concurrent> <olap-threads> <oltp-threads> <olaps-skew> <key-range-max> <root*-index> <gc> <gc-uip> <initial-keys-to-load>;# 
       ...
       <concurrent>: Enable concurrent execution of OLTP and OLAP threads: boolean. If false, then <oltp-threads> are the number of scans to execute after the OLTP workload completed
       ...
       <gc-uip>: Enable garbage collection with user input
       <initial-keys-to-load>: Number of initial keys to load
       Note: mv_text.rs -> Method "main_load" is the dispatcher for this command, also look at "main_load_yscb" for detailed implementations.

### For Example, replicating the paper results for Figures 16, 15, 12 and 11; use the following commands:
>### Concurrency level experiment (Figure 16): (Workload-Generation ./cMVBT generate 60.dat 10000 1000 200 600 200 0)
> 
    ...
    ./cMVBT load 60.dat true 16 32 0 max fg false false 10000; sleep 2;
...
> 
    ...
    ./cMVBT load 100.dat true 0 32 0 max fg false false 10000; sleep 2;

>### OLTP mixed workload with GC (Figure 12): (Data-Generation as above, vary the number of inserts/updates/deletes)
> 
    ...
    ./cMVBT load 100.dat true 16 32 0 max fg true false 10000; sleep 2;


>### OLTP mixed workload without GC (Figure 11): (Data-Generation as above, vary the number of inserts/updates/deletes)
> 
    ...
    ./cMVBT load 100.dat true 16 32 0 max fg false false 10000; sleep 2;
